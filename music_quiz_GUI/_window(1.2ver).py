...
class Walker2(QtCore.QThread):
    
    def __init__(self, parent=None):
        super(Walker2, self).__init__(parent)
        self.stopped = False
        self.mutex = QtCore.QMutex()
    
    def setup(self,data_length,n_components):
        self.stopped = False
        self.walker2_data_length = data_length
        self.walker2_n_components = n_components
    ...
    def run(self):
        ...
        #input voice data
        filenames = [('mixed/mixed' + str(i+1) + '.wav') for i in range(self.walker2_n_components)]
        mixed=[(wav.read(filenames[i])[1][0:self.walker2_data_length]) for i in range(self.walker2_n_components)]
        S = np.array(mixed).T
        
        ica = FastICA(n_components=self.walker2_n_components)
        ...
        filenames = [('separated/separated' + str(i+1) + '.wav') for i in range(self.walker2_n_components)]
        for i in range(self.walker2_n_components):
...
class MainWindow(QtGui.QWidget):
    ...
    def initUI(self,components):
        ...
        self.menu.sep_button.clicked.connect(self.start_separation)
        self.walker = Walker(self)
        self.walker2 = Walker2(self)
        self.walker2.finished.connect(self.show_titles)
        self.show()
    ...
    def start_separation(self):
        ...
        self.walker2.stop()
        self.walker2.wait()
        self.menu.sep_button.setText("計算中")
        self.menu.sep_button.setEnabled(False)
        self.walker2.setup(self.data_length,self.n_components)
        self.walker2.start()
    ...
    def show_titles(self):
        ...
        self.menu.sep_button.setText("分離・復元")
        self.menu.sep_button.setEnabled(True)
